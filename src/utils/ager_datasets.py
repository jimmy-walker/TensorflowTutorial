...
class DataManager(object):
    ...
    def _load_imdb(self):
        ...
        dob = dataset['imdb']['dob'][0, 0][0]  # Matlab serial date number
        photo_taken = dataset['imdb']['photo_taken'][0, 0][0]  # year
        age = [self._calc_age(photo_taken[i], dob[i]) for i in range(len(dob))]
        age_array = np.array(age)
        max_age_mask = age_array <= 100
        min_age_mask = 0 <= age_array
        face_score_mask = face_score > face_score_treshold
        second_face_score_mask = np.isnan(second_face_score)
        #unknown_gender_mask = np.logical_not(np.isnan(gender_classes))
        mask = np.logical_and(face_score_mask, second_face_score_mask)
        #mask = np.logical_and(mask, unknown_gender_mask)
        mask = np.logical_and(mask, max_age_mask)
        mask = np.logical_and(mask, min_age_mask)
        image_names_array = image_names_array[mask]
        #gender_classes = gender_classes[mask].tolist()
        age_values = age_array[mask].tolist()
        image_names = []
        for image_name_arg in range(image_names_array.shape[0]):
            image_name = image_names_array[image_name_arg][0]
            image_names.append(image_name)
        return dict(zip(image_names, age_values))
